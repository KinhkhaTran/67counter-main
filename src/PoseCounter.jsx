...
export default function PoseCounter() {
  ...
  // Keep ref in sync with state so pose callback can read it
  useEffect(() => {
  ...
  function startGame() {
    ...
    setGameActive(true);
    setImageSrc("/kai.webp");
    setImageName("Kai Cenat");
    setMaxProgress(10);
  ...
  useEffect(() => {
    ...
    pose.onResults((results) => {
      ...
      if (results.poseLandmarks) {
        ...
        if (isUpR && !forearmUp.current && gameActiveRef.current) {
  ...
  return (
    ...
    <div className="appContainer">
      <img src="/uvsa-midwest.webp" alt="UVSA-Midwest" className="logo" />
      ...
      {!gameActive && (
        <button className="btn-start" onClick={startGame}>
          {showTimesUp ? "Play Again" : "Start 30s Challenge"}
        </button>
      )}
